...
# Test build script for HelpMeTest CLI
# This script tests the build process locally before pushing to GitHub

set -e

echo "🧪 Testing HelpMeTest CLI Build Process"
echo "======================================="
...
# Test archive creation (simulate CI process)
...
cd test-archives
sha256sum *.tar.gz *.zip > helpmetest-cli_test_checksums.txt 2>/dev/null || sha256sum * > helpmetest-cli_test_checksums.txt
...
echo "🔐 Checksums:"
echo "============="
cat test-archives/helpmetest-cli_test_checksums.txt
